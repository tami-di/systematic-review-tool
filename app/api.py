    ...
    if (user_input): 
        ...
        paper_info = [
                {'title': str(paper[1]), 'library': str(paper[2]), 'code_name': str(paper[3]),
                 ...
                 'summary': str(paper[6]), 'source': str(paper[7]), 
                 'author': get_authors_from_paper_id_as_str(db, paper[0])} for paper in suggestions
            ]
...
#-------------------------Functions for Subcategory old api---------------------------
...
"""Funtion to add column to subcategory"""  ##no se si estara bien 
def add_column_to_subcategory(db, subcat_id, col_name, col_data):
    cursor = db.connection.cursor()
    # add column to table
    cursor.execute("SELECT extra FROM content WHERE id=%s",(subcat_id))
    extra=str(cursor.fetchall()[0][0])
    extra+=col_name+"$"+col_data+";"
    cursor.execute("UPDATE content SET extra=%s WHERE id=%s",(extra,subcat_id))
    # Commit changes in the database
    db.connection.commit()


def delete_row_from_subcategory(db, subcat_id, row_id): ##no se si estara bien
    cursor = db.connection.cursor()
    subcat_name = get_content_name_from_id(db,subcat_id)
    cursor.execute("SELECT id FROM "+subcat_name)
    cont_id= cursor.fetchall()[int(row_id)][0]
    cursor.execute("DELETE FROM "+subcat_name+" WHERE id=%s",(cont_id,))
    db.connection.commit()
