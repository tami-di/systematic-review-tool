...
def get_data_from_category_as_headers_and_column_data(db, cat_id):
    ...
    for cont_id in result:
        ...
        for row_data in row:
            ...
            extras = row_data[2].replace(" ", "-").split(';')  # Separar la columna 'extra' por el carácter ';'
            
            for extra, i in enumerate(extras, start=2):
                if extra < len(headers):
                    name = headers[extra]['name'].replace(" ", "-")
                    row_dict[name] = i.replace(" ", "-")  
    ...
    for category in categories_values:
        ...
        for element in category['values']:
            if element['is_subcat']:
                ...
                subcat_name = "papers.cat_cont"  
                interaction = element['rel_with_cat']
                cat_interact_subcat_table_name = "papers.int_cat"  
